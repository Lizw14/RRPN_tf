...
def train():
    with tf.Graph().as_default():
        ...
        print(rpn_proposals_boxes)
        print(rpn_proposals_scores)
        ...
        with tf.Session(config=config) as sess:
            ...
            shutil.copyfile(os.path.join(cfgs.SUMMARY_PATH, '../../libs/configs/cfgs.py'), os.path.join(summary_path, 'cfgs.py'))
            ...
            for step in range(cfgs.MAX_ITERATION):
                try:
                    ...
                    print('CORRECT: '+str(step)+'\t'+str(_img_name_batch[0]))
